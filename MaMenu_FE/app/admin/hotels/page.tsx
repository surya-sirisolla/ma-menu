...
import { Plus, Search, Building2, MapPin, User, Phone, Loader2, CheckCircle, Pencil, Trash2 } from 'lucide-react'
import { getHotels, createHotel, updateHotel, deleteHotel, getHotelOwners } from '@/lib/api'
...
const EMPTY_EDIT = { name: '', phone: '', email: '', address: '', city: '', state: '', country: '', pincode: '' }
...
export default function HotelsPage() {
  ...
  function openEdit(hotel: Hotel) {
    setEditTarget(hotel)
    setEditForm({ name: hotel.name, phone: hotel.phone ?? '', email: hotel.email ?? '',
  ...
  async function handleEdit(e: FormEvent) {
    ...
    try {
      await updateHotel(editTarget.id, editForm)
      setEditSuccess('Hotel updated successfully!')
      await load()
      setTimeout(() => { setEditOpen(false); setEditSuccess('') }, 1500)
    } catch (err: unknown) {
      setEditError(err instanceof Error ? err.message : 'Failed to update hotel')
    ...
    <div className="p-8">
      ...
      <div className="flex items-center gap-4 mb-6">
        <div className="relative flex-1 max-w-sm">
          ...
          <input type="text" className="input pl-9" placeholder="Search by name, city or country…"
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50/70">
                {['Hotel', 'Location', 'Owner', 'Contact', 'Status', 'Created', 'Actions'].map((h) => (
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={7} className="px-6 py-16 text-center">
                    ...
                    <p className="text-slate-400 text-sm">{search ? 'No hotels match your search' : 'No hotels yet'}</p>
                    {!search && (
                ...
                <tr key={h.id} className="hover:bg-slate-50/60 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    ...
                    {h.address && <p className="text-xs text-slate-400 mt-0.5 ml-5">{h.address}</p>}
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-1.5 text-slate-600">
                  ...
                  <td className="px-6 py-4">
                    {h.phone && <div className="flex items-center gap-1.5 text-slate-600"><Phone size={13} className="text-slate-400" />{h.phone}</div>}
                    {h.email && <p className="text-xs text-slate-400 mt-0.5">{h.email}</p>}
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => setDeleteTarget(h)} title="Deactivate"
          ...
          <form onSubmit={handleCreate} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="col-span-2">
                ...
                <input className="input" placeholder="Grand Palace Hotel" value={form.name}
              ...
              <div>
                ...
                <input type="email" className="input" placeholder="hotel@example.com" value={form.email}
      ...
      {/* edit modal */}
      <Modal open={editOpen} onClose={() => setEditOpen(false)} title={`Edit ${editTarget?.name ?? 'Hotel'}`} size="lg">
        {editSuccess ? (
          <div className="flex flex-col items-center py-6 gap-3">
            <CheckCircle size={44} className="text-emerald-500" />
          ...
          <form onSubmit={handleEdit} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1.5">State</label>
                <input className="input" value={editForm.state}
                  onChange={(e) => setEditForm({ ...editForm, state: e.target.value })} />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1.5">Country</label>
                <input className="input" value={editForm.country}
                  onChange={(e) => setEditForm({ ...editForm, country: e.target.value })} />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1.5">Pincode</label>
                <input className="input" value={editForm.pincode}
      ...
      {/* delete confirm */}
      <Modal open={!!deleteTarget} onClose={() => setDeleteTarget(null)} title="Deactivate Hotel" size="sm">
        <p className="text-slate-600 text-sm mb-6">
          Deactivate <strong>{deleteTarget?.name}</strong>? It will be hidden from the platform.
        </p>
